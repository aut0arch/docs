...
Built on **Next.js**, **React Flow**, and **Tailwind CSS**, the application creates a large interactive graph where nodes represent architectural clusters, and edges represent their flow and dependency.
...
## Core Technologies
* `next`: For the React framework and UI rendering.
* `@xyflow/react`: For rendering the mathematical node connections and interactive graph.
* `dagre`: For automatically organizing the nodes using hierarchical layout algorithms.
